...
async def stream_audio(project_name: str, file_name: str, request: Request):
    # Quét ưu tiên: Kho nhạc chính -> Outputs AI -> Inputs AI
    file_path = os.path.join(MUSIC_DIR, project_name, file_name)
...
async def get_cover_image(project_name: str, file_name: str):
    ...
    if not os.path.exists(file_path) or not file_name.endswith((".jpg", ".jpeg", ".png")):
        ...
        if os.path.exists(fallback_path):
            return FileResponse(path=fallback_path)
        else:
