...
def get_drive_service():
    if not os.path.exists(TOKEN_PATH):
        raise HTTPException(status_code=401, detail="Không tìm thấy token.json trong thư mục auth/.")
...
def check_status(admin=Depends(verify_admin_token)):
    ...
    return {
        ...
        "status_text": "Đã kết nối API" if is_auth else "Chưa kết nối API"
    }
...
def stop_task(task_id: str, admin=Depends(verify_admin_token)):
